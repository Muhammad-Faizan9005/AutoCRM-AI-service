...
class AutoCRMClient:
    ...
    async def list_stale_lead_candidates(self, limit: int = 500) -> list[dict[str, object]]:
        return await self._get_list(f"/api/agent/leads/stale-candidates?limit={limit}")
    ...
    async def get_user_summary_context(self, user_id: UUID | str, limit: int = 20) -> dict[str, object]:
        headers = await self.auth.get_async_headers()
        async with httpx.AsyncClient(timeout=settings.autocrm_auth_timeout, follow_redirects=True) as client:
            response = await client.get(
                f"{settings.autocrm_base_url}/api/agent/users/{user_id}/summary-context",
                params={"limit": limit},
                headers=headers,
            )
            response.raise_for_status()
    ...
    async def list_ai_agents(self) -> list[dict[str, object]]:
        ...
        return await self._get_list("/api/agent/ai-agents/runtime")
